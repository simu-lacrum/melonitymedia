// ─────────────────────────────────────────────────────────────
// Proxy rotation client — universal interface for mobile proxy providers.
//
// All providers expose the same operation (rotate IP for a single
// proxy slot), but the API contract differs. We normalize them
// behind a single rotateProxy() function dispatched by `provider`.
// ─────────────────────────────────────────────────────────────
...
export interface RotateProxyInput {
  ...
  externalId: string | null;     // slotId / modemId / proxyName
  apiKey: string | null;         // decrypted; only required for non-MANUAL with API
  rotationLink: string | null;   // for MANUAL with rotation URL provided by user
}
  ...
  try {
    switch (input.provider) {
      ...
      case 'PROXYS_IO': {
        ...
        // No-auth rotation endpoint; key is per-proxy, found in dashboard.
