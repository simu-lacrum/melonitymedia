...
| 🛡️ **Антидетект** | `undetected-chromedriver-js` — патченный ChromeDriver + Selenium WebDriver + Proxy Extension (Manifest V2) |
...
| **Worker** | BullMQ, `undetected-chromedriver-js` (не Puppeteer), Selenium WebDriver, Cheerio |
...
> **⚠️ Worker-контейнер** включает Xvfb + google-chrome-stable. Вместо Puppeteer используется `undetected-chromedriver-js` — обёртка над патченным ChromeDriver + Selenium WebDriver. Браузер запускается с `headless: false` внутри виртуального дисплея Xvfb `:99`, что позволяет обходить антифрод-детекцию TikTok и YouTube.
