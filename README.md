...
| 🔥 **Прогрев аккаунтов** | 10-дневный progressive curriculum (passive → active engagement) с **автоматическим продолжением** (self-rescheduling 20-28ч между днями) |
...
| 📊 **Аналитика** | curl-impersonate JSON API (~200ms/профиль) + **автоматический cron каждые 6ч** с persist в БД |
| 🛡️ **Антидетект** | Patchright (patched Playwright) + ghost-cursor + per-account fingerprints + typing emulator |
| 📱 **Mobile Fingerprint**| Поддержка mobile viewports, touch-событий и DPR в отпечатках |
...
| 🎥 **Video uniquification** | FFmpeg pipeline с детерминистичными transforms per account+video (pixel shift, hue, audio pitch) |
| 👤 **Edit Profile** | Смена никнейма, био и **аватара** через Patchright + ghost-cursor (TikTok / YouTube) |
...
| Слой | Технологии |
|------|-----------|
| **Frontend** | Next.js 15, React 19, Tailwind CSS v4, shadcn/ui (base-ui), Lucide Icons |
...
|---------|---------|---------|----------|
| `upload` | `upload.ts` | Кнопка «Запустить» | Patchright upload + video uniquification (FFmpeg, seed=accountId+videoPath) |
| `warmup` | `warmup.ts` | Кнопка (auto-continues) | 10-day progressive curriculum (passive → active), **self-rescheduling** на следующий день через 20-28ч |
...
| `edit-profile` | `edit-profile.ts` | Кнопка | Смена **аватара** (upload по URL), никнейма, био через ghost-cursor (TikTok + YouTube) |
