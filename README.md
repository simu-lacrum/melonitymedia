...
прогрева аккаунтов, фарминга cookies и сбора аналитики.
...
| Функция | Описание |
|---------|----------|
| 🎬 **Массовый залив** | Загрузка видео через антидетект-браузер с ротацией прокси |
| 🔥 **Прогрев аккаунтов** | Автоматический скроллинг, лайки, комментарии |
| 🍪 **Фарминг cookies** | Нагул и экспорт сессий для поддержания живых аккаунтов |
| 📊 **Аналитика** | Ночной cron-сбор подписчиков, просмотров, лайков через cheerio |
| 🛡️ **Антидетект** | UndetectedChrome с патченным драйвером + proxy-расширение Manifest V2 |
| 🌐 **Мобильные прокси** | Ротация IP через API-ссылки мобильных ферм (proxys.io) |
...
sequenceDiagram
    ...
    WK->>WK: Launch UndetectedChrome
    WK->>WK: Upload video to TikTok
...
| **Worker** | BullMQ, UndetectedChrome, Cheerio, Selenium WebDriver |
...
│   │   ├── prisma/
│   │   │   └── schema.prisma   # Database schema (tenant isolation)
│   │   └── package.json
│   │
...
│       ├── src/
│       │   ├── index.ts        # Worker entrypoint (6 queues)
│       │   ├── core/           # BrowserAutomation (UndetectedChrome)
...
cp .env.example .env
# Отредактировать .env — установить JWT_SECRET
...
graph LR
    subgraph Docker Compose
        ...
        WK[worker<br/>BullMQ + Chrome<br/>+ Xvfb]
    end
...
> **⚠️ Worker-контейнер** включает Xvfb + google-chrome-stable. Puppeteer запускается с `headless: false` внутри виртуального дисплея `:99` для обхода антифрод-детекции.
...
```mermaid
graph LR
    subgraph Queues["6 BullMQ Queues"]
        Q1[upload<br/>Залив видео]
        Q2[warmup<br/>Прогрев]
        Q3[cookies<br/>Фарминг cookies]
        Q4[edit-profile<br/>Редактирование]
        Q5[analytics-cron<br/>Парсинг статистики]
...
|---------|---------|---------|----------|
| `upload` | `upload.ts` | Кнопка «Запустить» | TikTok/YT Shorts загрузка через UC |
| `warmup` | `warmup.ts` | Кнопка / Cron | Скроллинг, лайки, комментарии |
| `cookies` | `cookies.ts` | Кнопка | Нагул cookies на сайтах-донорах |
| `edit-profile` | `edit-profile.ts` | Кнопка | Смена аватара, баннера, био |
| `analytics-cron` | `analytics.ts` | Cron (1 раз/ночь) | Парсинг подписчиков через cheerio |
...
Визуальная система основана на **Roboto Flex** (variable font) с высококонтрастной dark-theme палитрой:
...
| `--color-surface-elevated` | `#2d3139` | Приподнятые элементы |
| `--color-melon-pink` | `#ff1469` | Основной акцент (из логотипа) |
| `--color-ice-cyan` | `#40D3F5` | Вторичный акцент (из логотипа) |
| `--color-pure-white` | `#ffffff` | Текст |
...
```css
background: linear-gradient(135deg, #ff1469 0%, #40D3F5 100%);
```
...
|-------|--------|-------------|
| `.animate-float` | Парящее движение вверх-вниз | 6s |
| `.animate-float-slow` | Медленное парение с лёгким наклоном | 8s |
| `.animate-gradient` | Перемещение градиентного фона | 4s |
| `.animate-fade-up` | Появление снизу с fade-in | 0.8s |
| `.animate-glow-pulse` | Пульсация свечения (pink + cyan) | 3s |
| `.animate-spin-slow` | Медленное вращение (орбиты) | 20s |
| `.animate-shimmer` | Блеск/shimmer эффект | 2s |
...
| `POST` | `/api/proxies` | Добавить (host:port:login:pass + rotation link) |
...
| **No Fake UI** | Индикаторы загрузки до получения реального ответа API |
| **Confirm Dialogs** | Все деструктивные действия требуют подтверждения |
