...
| 👤 **Edit Profile** | Смена никнейма, био и **аватара** через Patchright + ghost-cursor (TikTok / YouTube), session warmup для YouTube Studio |
...
4. **Rebuild только app-сервисов** (`api`, `web`, `worker`) с `--force-recreate`
5. **db/redis** запускаются без force-recreate (сохранение данных)
6. **Image prune** после деплоя для экономии дискового пространства
...
| `edit-profile` | `edit-profile.ts` | Кнопка | Смена **аватара** (upload по URL/файлу), никнейма, био через ghost-cursor (TikTok + YouTube Studio), session warmup для YT |
