...
| Дни | Фаза | Действия |
|-----|-------|----------|
| 1-3 | Passive | Scroll FYP, имитация чтения комментариев, без взаимодействий |
| 4-6 | Light | Скролл, лайки (randomised), 1 комментарий |
| 7-10 | Active | Скролл, умная навигация (Shorts/Long-form), лайки, комментарии, сохранения, подписки |

> **Human-Like Behaviour (v0.4.0):** Во всех фазах внедрены реалистичные движения мыши (`ghost-cursor`). Прогрев теперь поддерживает **multi-session** (несколько сессий в день с перерывами на сон — sleep scheduling), что делает поведение неотличимым от реального человека.

Гард:
- Аккаунт с `warmupCompletedAt === null` НЕ допускается в очередь `upload`
- Статус `WARMING_UP` → `ALIVE` автоматически при `warmupDay >= 10`
- Режим `warmupMode="HOURS"` является быстрым диагностическим прогревом: после него аккаунт остаётся `PAUSED`, `warmupCompletedAt` не выставляется, upload-gate не открывается автоматически.
...
**Механизм:** После завершения дня N (когда N < totalDays), warmup handler автоматически ставит в очередь BullMQ следующую сессию. Внутри дня перерывы 1.5-4 часа; после последней сессии дня следующий день планируется через 6-8 часов сна.
...
2. IF N >= totalDays:
   ...
   - Прогрев завершён
3. ELSE IF это не последняя сессия дня:
   - Вычисляется перерыв: 90-240 минут
   - BullMQ.add('warmup', { accountId, warmupDays, warmupDay, sessionInDay + 1 }, { delay })
   - Лог: "Сессия N/M запланирована"
4. ELSE:
   - Вычисляется сон между днями: 6-8 часов
   - BullMQ.add('warmup', { accountId, warmupDays, sessionInDay: 0 }, { delay })
   - Лог: "Следующий день прогрева запланирован"
```

**Почему multi-session:** Реальный пользователь заходит несколько раз в день короткими сессиями, а не одним ровным запуском раз в сутки. Рандомизированные паузы и сон между днями снижают риск паттерна "один запуск по расписанию".
...
Warmup использует **только** пользовательские комментарии из `data.comments`. Hardcoded comment pool удалён: если список комментариев пустой или не передан, worker не открывает форму комментария и не публикует текст. Это нужно, чтобы пользователь точно контролировал, что именно пишет аккаунт, и чтобы поведение не смешивало ниши разными шаблонными фразами.
...
### 9.6. Web Components Bypass и Навигация (v0.4.0)

- **Прямая навигация:** Для обхода сложных UI-флоу YouTube Studio используется прямой переход по URL `?d=ud`, который сразу открывает диалог загрузки видео.
- **Shadow DOM:** Web components (например, `ytcp-uploads-dialog`) пробиваются с помощью `page.evaluate`, так как стандартные Playwright-селекторы часто нестабильны внутри shadow DOM.
- **Смена аватара:** Для обхода антифрода при смене аватара используется встроенный Google Account iframe flow (myaccount.google.com), а не попытка изменить его через хрупкий интерфейс Studio.
...
### 10.3. Strict Regex и Google 2FA (v0.4.0)

- **Google Device 2FA:** Добавлена полноценная поддержка подтверждения входа через уведомления на устройствах Google (Tap "Yes" on your phone).
- **Strict Regex:** Усилены регулярные выражения для парсинга состояния 2FA, чтобы избежать жадного захвата (greedy matching) и предотвратить ложные срабатывания (false positives).

### 10.4. Ghost-Cursor Patchright Shim
...
## 11. Manual Verification Bypass (VNC & noVNC)

**Контекст:** В некоторых случаях (например, новые типы капч, верификация по номеру телефона, подозрительная активность) автоматика не может пройти проверку безопасности.

**Механизм (v0.4.1):**
- Xvfb, x11vnc и websockify запускаются динамически для каждого browser job.
- noVNC не публикуется наружу: пользователь открывает монитор только через `/api/workspace/jobs/:taskId/monitor/:jobId`.
- API проверяет JWT и `VncSession.userId`, поэтому пользователь видит только свои активные мониторы.
- Несколько зависших задач отображаются как отдельные `vncSessions` в `/api/workspace/jobs`; их можно открыть по очереди из UI.
