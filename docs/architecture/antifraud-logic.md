...
**Механизм (v0.4.0):**
- Добавлена поддержка VNC (порт 5900) и noVNC (порт 6080) для каждого воркера.
- Позволяет пользователю вручную подключиться к браузеру, работающему в контейнере (внутри Xvfb), и пройти верификацию.
- Изоляция per-job гарантирует, что пользователь управляет сессией только конкретного аккаунта.
- Данный подход является ультимативным решением (manual bypass) для сложного антифрода без вызова подозрений у платформы.
