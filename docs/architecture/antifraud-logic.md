# Антифрод-логика MelonityMedia v3.1 — Техническая спецификация
...
### 2.4. Два подхода в коде

В файле существуют ДВА способа детекции:

1. **`shadowbanDetectorHandler`** — **☠️ DEPRECATED** — legacy, использует curl-impersonate для live-запросов к TikTok API. Подлежит удалению в следующем рефакторе (`chore(worker): remove legacy curl-impersonate shadowban path`).
2. **`detectShadowbanForAccount`** — **✅ Актуальный (v3.1)**, использует сохранённые данные из БД (Prisma)

**Новый подход — единственный рекомендованный.** Преимущества:
- Не требует cookies (не палит сессию)
- Не создаёт лишних запросов к TikTok
- Работает за ~5ms вместо ~30s
- Live-проверка реальных views — это отдельная очередь `analytics-cron`, не shadowban-логика

> ⚠️ **TODO (tech debt):** Удалить `shadowbanDetectorHandler` и оставить только `detectShadowbanForAccount`.
...
- Chrome 100 в UA при установленном Chrome 148 = top-tier сигнал
...
3. Fallback: 148 (с предупреждением)
...
**Проблема:** Если worker обновили (Chrome 148 → 149), а fingerprint в БД старый (`chromeMajor=148`), валидация при загрузке обнаружит несоответствие.
...
## 4. Cookie Encryption (AES-256-GCM)
