...
- **Прямая навигация:** Чтобы не зависеть от хрупкого многошагового UI-флоу YouTube Studio, используется прямой переход по URL `?d=ud`, который сразу открывает диалог загрузки видео.
...
- **Смена аватара:** Для устойчивой смены аватара используется встроенный Google Account iframe flow (myaccount.google.com), а не попытка изменить его через хрупкий интерфейс Studio.
