...
1. **`shadowbanDetectorHandler`** — legacy, использует curl-impersonate для live-запросов к TikTok API
2. **`detectShadowbanForAccount`** — новый (v3.1), использует сохранённые данные из БД (Prisma)

Новый подход предпочтителен: не требует cookies, не создаёт лишних запросов к TikTok, работает быстрее.
