...
> **Быстрый тест (часы)** в `/account/workspace` нужен только для короткой активности и проверки сессии. Он завершает задачу, но оставляет аккаунт `PAUSED` и не выставляет `warmupCompletedAt`; для безопасных заливов используйте полноценный режим по дням.
