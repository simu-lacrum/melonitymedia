...
2. Заполните поля:
   - **Type:** `LTE_MOBILE` для всех аккаунтов младше 30 дней. `STATIC_RESIDENTIAL` допустим только для аккаунтов старше 30 дней и только если страна операций не меняется.
...
3. **Авто-продолжение:** worker сам планирует 2-4 сессии в день. Между дневными сессиями пауза 1.5-4ч, после последней сессии следующий день планируется через 6-8ч сна.
4. Статус аккаунта покажет прогресс прогрева `X / 10`.
5. Когда `warmupCompletedAt != null` → аккаунт допускается в очередь `upload`.

> **Ускоренный режим (часы)** в `/account/workspace` засчитывает прогрев после выбранного количества часов и открывает заливы. Это быстрее, но рискованнее для свежих аккаунтов, чем режим по дням; используйте его осознанно.
